...
def plot_spec(spc, peaks=None):
    """simple plotting routine for spectra """
    counts = np.array(spc.counts).astype(int)
    plt.clf()

    if spc.ebins is None:
...
def get_peak_roi(peak_pos, spc, offset=10):
    ...
    """
    y = spc.counts[peak_pos-offset:peak_pos+offset]
    x = spc.ebins[peak_pos-offset:peak_pos+offset]
...
def rebin(x, bin_ratio=10):
    # rebin
    nbins = len(x) * bin_ratio
    x = np.arange(x[0], x[-1], (x[-1]-x[0])/nbins) 
    return x
...
def get_spect(path):
    ...
    spc.counts = get_counts(lines)
    print(spc.counts)
    spc.ebins = generate_ebins(lines, len(spc.counts))
    spc.live_time = get_live_time(lines)
    spc.real_time = get_real_time(lines)
